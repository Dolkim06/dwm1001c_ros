#! /usr/bin/env python3
# tag_position을 Pose 메시지 타입으로 발행
...
from geometry_msgs.msg import Pose  # 변경: Pose 메시지 사용
...
class Ranges:
    def __init__(self):
        ...
        # 태그 위치 데이터 토픽 (Pose 메시지 사용)
        self.tagPosPub = rospy.Publisher("/tag_position", Pose, queue_size=10)
                    ...
                    if "DIST" in raw_data:
                        ...
                        try:
                            ...
                            if "POS" in parts:
                                ...
                                # 태그 위치 메시지 생성 및 발행 (Pose 메시지 사용)
                                tag_pose = Pose()
                                tag_pose.position.x = tag_x
                                tag_pose.position.y = tag_y
                                tag_pose.position.z = tag_z
                                # orientation은 기본값 유지

                                self.tagPosPub.publish(tag_pose)
